...
分割や IDE 風レイアウトで開くペインは、元のペインの作業ディレクトリを引き継ぎます。Windows のローカルペインでは WezTerm がプロセス情報から現在地を推定するので、そのままでも動きます。推定できない環境（SSH 先、マルチプレクサ経由など）では、シェルが現在地を通知している（OSC 7）必要があり、通知が無いと新しいペインはホームディレクトリで開きます。タブに出る作業ディレクトリ名も同じ仕組みです。
